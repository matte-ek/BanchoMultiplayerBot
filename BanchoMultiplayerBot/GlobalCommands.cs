    ...
    private async void OnPrivateMessageReceived(IPrivateIrcMessage msg)
    {
        try
        {
            if (msg.Content.ToLower().Equals("!help") || msg.Content.ToLower().Equals("!info") ||
                msg.Content.ToLower().Equals("!commands"))
            {
                _bot.SendMessage(msg.IsDirect ? msg.Sender : msg.Recipient,
                    $"osu! auto host rotation bot (v{Bot.Version}) [https://github.com/matte-ek/BanchoMultiplayerBot/blob/master/COMMANDS.md Help & Commands]");
            }
            ...
            if (msg.Content.ToLower().Equals("!rs"))
            {
                ...
                var ppInformation =
                    await _bot.PerformancePointCalculator!.CalculateScorePerformancePoints(
                        int.Parse(recentScore.BeatmapId!), recentScore);
                if (ppInformation == null)
                {
                    _bot.SendMessage(msg.IsDirect ? msg.Sender : msg.Recipient,
                        "Unable to get calculate pp information.");
                    return;
                }
                ...
                if (recentScore.Perfect != "1" &&
                    ppInformation.MaximumPerformancePoints != ppInformation.PerformancePoints)
                {
                ...
                response.Append(
                    $"x{recentScore.Maxcombo}/{beatmapInformation.MaxCombo} | {recentScore.Count300}/{recentScore.Count100}/{recentScore.Count50}/{recentScore.Countmiss}");
            ...
            if ((msg.Content.ToLower().Equals("!map") || msg.Content.ToLower().Equals("!m")) && msg.IsDirect)
            {
                ...
                var ppInformation =
                    await _bot.PerformancePointCalculator!.CalculatePerformancePoints(
                        int.Parse(recentScore.BeatmapId!));
                if (ppInformation == null)
                {
                    _bot.SendMessage(msg.IsDirect ? msg.Sender : msg.Recipient,
                        "Unable to get calculate pp information.");
                ...
                var starRating =
                    Math.Round(float.Parse(beatmapInformation.DifficultyRating!, CultureInfo.InvariantCulture), 2);
                var length = TimeSpan.FromSeconds(int.Parse(beatmapInformation.TotalLength ?? "0",
                    CultureInfo.InvariantCulture));

                _bot.SendMessage(msg.IsDirect ? msg.Sender : msg.Recipient,
                    $"[https://osu.ppy.sh/b/{recentScore.BeatmapId} {beatmapInformation.Artist} - {beatmapInformation.Title} [{beatmapInformation.Version ?? string.Empty}]]");
                _bot.SendMessage(msg.IsDirect ? msg.Sender : msg.Recipient,
                    $"(Star Rating: {starRating:.0#} | {beatmapInformation.GetStatusString()} | Length: {length:mm\\:ss} | BPM: {beatmapInformation.Bpm})");
                _bot.SendMessage(msg.IsDirect ? msg.Sender : msg.Recipient,
                    $"(AR: {beatmapInformation.DiffApproach} | CS: {beatmapInformation.DiffSize} | OD: {beatmapInformation.DiffOverall} | HP: {beatmapInformation.DiffDrain} | 100%: {ppInformation.Performance100}pp | 98%: {ppInformation.Performance98}pp | 95%: {ppInformation.Performance95}pp)");
            }
        }
        catch (Exception e)
        {
